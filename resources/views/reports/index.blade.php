...
<x-app-layout>
    <div class="space-y-6">
        ...
        <section class="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
            @foreach ([
                ['route' => 'reports.appointments', 'permission' => 'reports.appointments', 'title' => 'Reporte de citas', 'text' => 'Agenda, estados y asistencia.'],
                ['route' => 'reports.clinical', 'permission' => 'reports.clinical', 'title' => 'Reporte clínico', 'text' => 'Consultas, recetas e historiales.'],
                ['route' => 'reports.financial', 'permission' => 'reports.financial', 'title' => 'Reporte financiero', 'text' => 'Ingresos, cartera y métodos de pago.'],
                ['route' => 'reports.patients', 'permission' => 'reports.patients', 'title' => 'Reporte de pacientes', 'text' => 'Crecimiento y actividad de pacientes.'],
                ['route' => 'reports.doctors', 'permission' => 'reports.doctors', 'title' => 'Reporte de médicos', 'text' => 'Rendimiento y producción médica.'],
                ['route' => 'reports.services', 'permission' => 'reports.services', 'title' => 'Reporte de servicios', 'text' => 'Uso, precios e ingresos por servicio.'],
            ] as $report)
        ...
        <section class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            @can('patients.view') @include('reports._stat-card', ['label' => 'Pacientes activos', 'value' => number_format($metrics['activePatients']), 'summary' => number_format($metrics['newPatients']).' nuevos en el periodo']) @endcan
            @can('doctors.view') @include('reports._stat-card', ['label' => 'Médicos activos', 'value' => number_format($metrics['activeDoctors']), 'tone' => 'green']) @endcan
            @can('services.view') @include('reports._stat-card', ['label' => 'Servicios activos', 'value' => number_format($metrics['activeServices']), 'tone' => 'slate']) @endcan
            @can('reports.appointments') @include('reports._stat-card', ['label' => 'Citas del periodo', 'value' => number_format($metrics['appointments']), 'summary' => number_format($metrics['todayAppointments']).' citas de hoy']) @endcan
            @can('reports.clinical') @include('reports._stat-card', ['label' => 'Consultas realizadas', 'value' => number_format($metrics['consultations']), 'tone' => 'green']) @endcan
            @can('reports.clinical') @include('reports._stat-card', ['label' => 'Recetas emitidas', 'value' => number_format($metrics['prescriptions']), 'tone' => 'blue']) @endcan
            @can('reports.financial') @include('reports._stat-card', ['label' => 'Ingresos pagados', 'value' => '$'.number_format($metrics['paidIncome'], 2), 'tone' => 'green', 'summary' => 'Solo pagos con estado pagado']) @endcan
            @can('reports.financial') @include('reports._stat-card', ['label' => 'Pagos pendientes', 'value' => number_format($metrics['pendingPayments']), 'tone' => 'yellow', 'summary' => number_format($metrics['cancelledOrRefundedPayments']).' cancelados o reembolsados']) @endcan
        </section>
