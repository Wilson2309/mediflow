<x-app-layout>
    <div class="space-y-6">
        <header class="flex flex-col gap-4 rounded-lg border border-[#E2E8F0] bg-white p-5 shadow-sm lg:flex-row lg:items-center lg:justify-between">
            ...
            <a href="{{ route('prescriptions.create') }}" class="inline-flex items-center justify-center rounded-lg bg-[#2563EB] px-4 py-3 text-sm font-semibold text-white shadow-sm shadow-blue-500/20 transition hover:bg-blue-700">Nueva receta</a>
        </header>
                            ...
                            <tr class="hover:bg-[#F8FAFC]">
                                ...
                                <td class="px-5 py-4">
                                    <div class="flex justify-end gap-2">
                                        ...
                                        <a href="{{ route('prescriptions.edit', $prescription) }}" class="rounded-lg border border-[#E2E8F0] px-3 py-2 text-xs font-semibold text-[#475569]">Editar</a>
                                        <form method="POST" action="{{ route('prescriptions.destroy', $prescription) }}" onsubmit="return confirm('¿Eliminar esta receta?');">
                                            @csrf
                                            @method('DELETE')
