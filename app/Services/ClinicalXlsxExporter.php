...
class ClinicalXlsxExporter
{
    public function stream(Collection $consultations, array $metrics, string $periodLabel, Carbon $generatedAt, ?User $generatedBy, string $timezone): void
    {
        echo $this->build($consultations, $metrics, $periodLabel, $generatedAt, $generatedBy, $timezone);
    }

    public function build(Collection $consultations, array $metrics, string $periodLabel, Carbon $generatedAt, ?User $generatedBy, string $timezone): string
    {
        return $this->zip($this->files($consultations, $metrics, $periodLabel, $generatedAt, $generatedBy, $timezone));
    }

    private function files(Collection $consultations, array $metrics, string $periodLabel, Carbon $generatedAt, ?User $generatedBy, string $timezone): array
    {
        return [
            ...
            'xl/worksheets/sheet1.xml' => $this->worksheetXml($consultations, $metrics, $periodLabel, $generatedAt, $generatedBy, $timezone),
        ];
    }
    ...
    private function worksheetXml(Collection $consultations, array $metrics, string $periodLabel, Carbon $generatedAt, ?User $generatedBy, string $timezone): string
    {
        $rows = [];
        $rows[] = $this->row(1, [$this->inlineCell('A1', 'Reporte clínico', 1)]);
        ...
        $rows[] = $this->row(7, [
            $this->inlineCell('A7', 'Consultas', 3),
            $this->inlineCell('B7', 'Recetas emitidas', 3),
            $this->inlineCell('C7', 'Con historial clínico', 3),
            $this->inlineCell('D7', 'Sin historial clínico', 3),
        ]);
        $rows[] = $this->row(8, [
            $this->numberCell('A8', (int) ($metrics['consultations'] ?? 0), 4),
            $this->numberCell('B8', (int) ($metrics['prescriptions'] ?? 0), 4),
            $this->numberCell('C8', (int) ($metrics['withMedicalRecord'] ?? 0), 4),
            $this->numberCell('D8', (int) ($metrics['withoutMedicalRecord'] ?? 0), 4),
        ]);

        $headers = ['Nro. consulta', 'Fecha', 'Paciente', 'Médico', 'Diagnóstico', 'Tratamiento'];
        $headerCells = [];
        ...
        foreach ($consultations as $consultation) {
            $rows[] = $this->row($rowNumber, [
                $this->inlineCell('A'.$rowNumber, 'CON-'.str_pad((string) $consultation->id, 6, '0', STR_PAD_LEFT), 7),
                $this->inlineCell('B'.$rowNumber, $consultation->consultation_date?->format('d/m/Y H:i') ?? 'Sin fecha', 8),
                $this->inlineCell('C'.$rowNumber, $consultation->patient?->full_name ?? 'Sin paciente', 6),
                $this->inlineCell('D'.$rowNumber, $consultation->doctor?->user?->name ?? 'Sin médico', 6),
                $this->inlineCell('E'.$rowNumber, $consultation->diagnosis ?: 'Sin diagnóstico', 6),
                $this->inlineCell('F'.$rowNumber, $consultation->treatment ?: 'Sin tratamiento', 6),
            ]);
            $rowNumber++;
        }
        ...
        return '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>'.
            ...
            '<cols><col min="1" max="1" width="16" customWidth="1"/><col min="2" max="2" width="20" customWidth="1"/><col min="3" max="3" width="30" customWidth="1"/><col min="4" max="4" width="28" customWidth="1"/><col min="5" max="5" width="40" customWidth="1"/><col min="6" max="6" width="40" customWidth="1"/></cols>'.
            '<sheetData>'.implode('', $rows).'</sheetData>'.
            '<autoFilter ref="A10:F'.$lastRow.'"/>'.
            '<mergeCells count="5"><mergeCell ref="A1:F1"/><mergeCell ref="A2:F2"/><mergeCell ref="A3:F3"/><mergeCell ref="A4:F4"/><mergeCell ref="A6:F6"/></mergeCells>'.
    ...
    private function workbookXml(): string
    {
        return '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>'.
            ...
            '<sheets><sheet name="Reporte clínico" sheetId="1" r:id="rId1"/></sheets>'.
            '</workbook>';
    }
